...
if [ ! -f /oar_provisioned ]; then
	...
	oarproperty -a halfcpu || true
	    ...
	    if ((core <= cores/2 + (cpu-1)*cores)); then
		    oarnodesetting -a -h "dev-node-"$node -p host="dev-node-"$node -p cpu=$cpu -p core=$core -p cpuset=$(((core-1)%cores)) -p mem=$mem -p halfcpu=$cpu &
	    else
		    oarnodesetting -a -h "dev-node-"$node -p host="dev-node-"$node -p cpu=$cpu -p core=$core -p cpuset=$(((core-1)%cores)) -p mem=$mem -p halfcpu=-$cpu &
	    fi
            wait
        ...
        for ((core=1;core<=$totalcores; core++)); do
            ...
            echo $node $cpu $core $(((core-1)%cores)) >> $log
	    if ((core <= cores/2 + (cpu-1)*cores)); then
		    oarnodesetting -r $core -p "cpu=$cpu" -p "core=$core" -p "cpuset=$(((core-1)%cores))" -p mem=$mem -p "halfcpu=$cpu" &
	    else
		    oarnodesetting -r $core -p "cpu=$cpu" -p "core=$core" -p "cpuset=$(((core-1)%cores))" -p mem=$mem -p "halfcpu=-$cpu" &
	    fi
            wait
